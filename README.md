...
| **Asset Type** | Consumer Loan Facility |
| **Control Record** | Consumer Loan Facility Fulfillment Arrangement |
...
| POST | `/v1/consumer-loan-facility-fulfillment-arrangement/initiate` | Initiate |
| GET | `/v1/consumer-loan-facility-fulfillment-arrangement` | Retrieve (list) |
| GET | `/v1/consumer-loan-facility-fulfillment-arrangement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/consumer-loan-facility-fulfillment-arrangement/{crId}/update` | Update |
| PUT | `/v1/consumer-loan-facility-fulfillment-arrangement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/consumer-loan-facility-fulfillment-arrangement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-consumer-loan:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-consumer-loan ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-consumer-loan` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
